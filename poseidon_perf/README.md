...
Small Noir circuit that stress-tests the Poseidon2 hash by hashing a configurable grid of inputs (`NUM_ITERATIONS` in `src/main.nr`) and returning all digests.
...
Example:
```bash
PACKAGE=poseidon_perf \           # override package (defaults to poseidon_perf)
...
VK_DIR=target/poseidon_perf.vk \  # where to cache/read the verification key dir
